...
def createKriging(setting, *args):
    ...
    opt = optimize.differential_evolution(lambda x: algKriging.thetaFunc(x, F, R, setting), bounds=setting['bounds'],
                                          strategy='best2bin')
    print opt
...
def predictor(x_pred, model, setting, *args):
    ...
    mu = npy.matmul(npy.transpose(f), model['beta']) \
         + npy.matmul(npy.matmul(npy.transpose(r_pred), npy.linalg.inv(model['R_pred'])),
                      setting['exp_y'] - npy.matmul(model['F'], model['beta']))
    ...
    mat2 = npy.linalg.inv(
        npy.matmul(npy.matmul(npy.transpose(model['F']), npy.linalg.inv(model['R_pred'])), model['F']))
    ...
    ci = npy.array([mu.reshape(len(mu), 1) - 1.96 * npy.sqrt(npy.diag(var_pred).reshape(len(mu), 1)), \
                    mu.reshape(len(mu), 1) + 1.96 * npy.sqrt(npy.diag(var_pred).reshape(len(mu), 1))]).reshape(2,
                                                                                                               len(mu))
    ci = npy.transpose(ci)
