...
def quadrature(n_deg, x_prob, full_model):
    ...
    alpha_total = npy.zeros(
        (math.factorial(n_deg + len(x_prob)) / math.factorial(n_deg) / math.factorial(len(x_prob)), 1))
    basis_total = npy.ones(((n_deg + 1) ** len(x_prob),
                            math.factorial(n_deg + len(x_prob)) / math.factorial(n_deg) / math.factorial(len(x_prob)),
                            1))
    integration_total = npy.ones(((n_deg + 1) ** len(x_prob),
                                  math.factorial(n_deg + len(x_prob)) / math.factorial(n_deg) / math.factorial(
                                      len(x_prob)), 1))
    ...
    for key, value in x_prob.iteritems():
        ...
        [basis[:, :, i], integration[:, :, i]] = algPCE.gen_basis(n_deg, key, value, alpha, beta, x_quad[:, i],
                                                                  weight[:, i])
...
def collocation(n_deg, x_prob, x_exp, y_exp, meta_type):
    ...
    basis_coef_total = npy.ones(
        (len(x_exp), math.factorial(n_deg + len(x_prob)) / math.factorial(n_deg) / math.factorial(len(x_prob)), 1))
    ...
    for i in xrange(0, len(x_exp)):
        for iters in xrange(0,
            ...
            for j in xrange(0, len(x_prob)):
                basis_coef_total[i, iters, 0] = basis_coef_total[i, iters, 0] * basis_coef[
