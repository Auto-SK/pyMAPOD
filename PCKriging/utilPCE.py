...
def predictor(PCE, x_pred):
    ...
    basis_total = npy.ones((len(x_pred), math.factorial(PCE['n_deg'] + len(PCE['x_prob'])) / math.factorial(
        PCE['n_deg']) / math.factorial(len(PCE['x_prob']))))
    ...
    for i in xrange(0, len(x_pred[:, 0])):
        for iters in xrange(0, math.factorial(PCE['n_deg'] + len(PCE['x_prob'])) / math.factorial(
                PCE['n_deg']) / math.factorial(len(PCE['x_prob']))):
            for j in xrange(0, len(PCE['x_prob'])):
                basis_total[i, iters] = basis_total[i, iters] * basis[
                    i, int(PCE['trunc_index'][iters * len(PCE['x_prob']) + j]), j]
