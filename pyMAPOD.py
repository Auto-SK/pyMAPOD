...
if input_type == 'dat':
    ...
    if calc_sobol == True:
        ...
        if calc_sobol_meta == False:
            if len(a) == 0:
                [Sobol_1st, Sobol_total] = sens_analysis.Sobol(a, x_prob, True, 1000, 0.05, x_exp1, x_exp2, funcForm,
                                                               funcName)
            else:
                ...
                for i in xrange(0, len(a)):
                    [Sobol_1st, Sobol_total] = sens_analysis.Sobol([a[i]], x_prob, True, 1000, 0.05, x_exp1, x_exp2,
                                                                   funcForm, funcName)
    ...
    if PCE_gen == True:
        ...
        if gen_metamodel == True:
            ...
            if len(a) == 0:
                [Sobol_1st, Sobol_total] = sens_analysis.Sobol(a, x_prob, True, 1000, 0.05, x_exp1, x_exp2, n_deg,
                                                               x_exp, data, meta_type)
            else:
                sobol_1st_1 = npy.zeros((len(a), 3))
                sobol_1st_2 = npy.zeros((len(a), 3))
                sobol_total_1 = npy.zeros((len(a), 3))
        ...
        if gen_metamodel == True:
            ...
            if len(a) == 0:
                [Sobol_1st, Sobol_total] = sens_analysis.Sobol(a, x_prob, True, 1000, 0.05, x_exp1, x_exp2, x_exp, data,
                                                               setting, lower_bound, upper_bound)
            else:
                sobol_1st_1 = npy.zeros((len(a), 3))
                sobol_1st_2 = npy.zeros((len(a), 3))
                sobol_total_1 = npy.zeros((len(a), 3))
                ...
                for i in xrange(0, len(a)):
                    y_exp = data[i * len(x_exp):(i + 1) * len(x_exp), :]
                    [Sobol_1st, Sobol_total] = sens_analysis.Sobol([a[i]], x_prob, True, 1000, 0.05, x_exp1, x_exp2,
                                                                   x_exp, y_exp, setting, lower_bound, upper_bound)
                    sobol_1st_1[i, :] = Sobol_1st[0, :]
                    sobol_1st_2[i, :] = Sobol_1st[1, :]
                    sobol_total_1[i, :] = Sobol_total[0, :]
                    sobol_total_2[i, :] = Sobol_total[1, :]
        ...
        if gen_metamodel == True:
            ...
            rmse, nrmse = pyPCKriging.meta_val(n_deg, x_val, y_real, x_exp, data, x_prob, a, setting, lower_bound,
                                               upper_bound)
        elif calc_sobol_meta == True:
            if len(a) == 0:
                [Sobol_1st, Sobol_total] = sens_analysis.Sobol(a, x_prob, True, 1000, 0.05, x_exp1, x_exp2, n_deg,
                                                               x_exp, data, setting, lower_bound, upper_bound)
            else:
                sobol_1st_1 = npy.zeros((len(a), 3))
                sobol_1st_2 = npy.zeros((len(a), 3))
                sobol_total_1 = npy.zeros((len(a), 3))
                ...
                for i in xrange(0, len(a)):
                    y_exp = data[i * len(x_exp):(i + 1) * len(x_exp), :]
                    [Sobol_1st, Sobol_total] = sens_analysis.Sobol([a[i]], x_prob, True, 1000, 0.05, x_exp1, x_exp2,
                                                                   n_deg, x_exp, y_exp, setting, lower_bound,
                                                                   upper_bound)
                    sobol_1st_1[i, :] = Sobol_1st[0, :]
                    sobol_1st_2[i, :] = Sobol_1st[1, :]
                    sobol_total_1[i, :] = Sobol_total[0, :]
